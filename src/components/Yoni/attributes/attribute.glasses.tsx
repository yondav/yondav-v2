...
export default function Glasses({ type, fill }: YoniAttributeProps<GLASSESTYPE>) {
  const elementId = useMemo(() => `glasses${type ? `-${type}` : ''}`, [type]);
  ...
  const elements: {
    ...
    const versaceShades = {
      bridge: {
        fill: fill?.primary ?? DEFAULTCOLOR.GLASSESSHADES1,
